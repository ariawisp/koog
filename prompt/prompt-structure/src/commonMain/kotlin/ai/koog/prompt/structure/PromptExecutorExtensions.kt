...
/**
 * Default mapping of some providers to their respective known simple JSON schema format generators.
...
public val DefaultSimpleSchemaGenerators: Map<LLMProvider, JsonSchemaGenerator> = mapOf(
    LLMProvider.OpenAI to OpenAISimpleJsonSchemaGenerator,
    LLMProvider.Google to GoogleSimpleJsonSchemaGenerator
)

/**
 * Default mapping of some providers to their respective known full JSON schema format generators.
 * Used to attempt to get a proper generator in the simple version of [executeStructured] (that does not accept [StructuredOutput] explicitly)
 * to attempt to generate an appropriate schema for the passed [KType].
 */
public val DefaultFullSchemaGenerators: Map<LLMProvider, JsonSchemaGenerator> = mapOf(
    LLMProvider.OpenAI to OpenAIFullJsonSchemaGenerator,
    LLMProvider.Google to GoogleFullJsonSchemaGenerator
)
...
public suspend fun <T> PromptExecutor.executeStructured(
    ...
    val structuredOutput = when {
        LLMCapability.Schema.JSON.Full in model.capabilities -> StructuredOutput.Native(
            JsonStructuredData.createJsonStructure(
                id = id, serializer = serializer,
                schemaGenerator = DefaultFullSchemaGenerators[model.provider] ?: FullJsonSchemaGenerator
            )
        )

        LLMCapability.Schema.JSON.Simple in model.capabilities -> StructuredOutput.Native(
            JsonStructuredData.createJsonStructure(
                id = id, serializer = serializer,
                schemaGenerator = DefaultSimpleSchemaGenerators[model.provider] ?: OpenAISimpleJsonSchemaGenerator
            )
        )
