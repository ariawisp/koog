use jni::JNIEnv;
use jni::objects::{JClass, JString, JIntArray, JObject};
use jni::sys::{jintArray, jlong, jstring, jobject};
use openai_harmony::{HarmonyEncoding, HarmonyEncodingName, load_harmony_encoding, StreamableParser};
use openai_harmony::chat::{Conversation, Message, Role, Author, SystemContent, DeveloperContent, TextContent};
...
pub extern "system" fn Java_ai_koog_prompt_harmony_HarmonyJNIBridge_createStreamingParser(
    ...
    let result = panic::catch_unwind(|| {
        ...
        let parser = StreamableParser::new(encoding, parse_role);
