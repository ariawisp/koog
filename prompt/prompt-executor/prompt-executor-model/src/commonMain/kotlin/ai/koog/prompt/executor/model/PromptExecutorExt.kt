...
public object PromptExecutorExt {
     ...
     * @return A single `Response` representing the result generated by the language model.
     */
    public suspend fun PromptExecutor.execute(prompt: Prompt, model: LLModel): Response {
     ...
     */
    public fun List<Response>.singleResponseOrNull(): Response? {
        if (this.size > 1) {
     ...
     * @throws NoSuchElementException if the list is empty.
     */
    public fun List<Response>.firstResponse(): Response {
        if (this.isEmpty()) {
            throw NoSuchElementException("No response received from LLM")
        }
     ...
     */
    public fun List<Response>.firstResponseOrNull(): Response? {
        if (this.size > 1) {
            logger.warn { "More than one response found: $this" }
        }
