...
import ai.koog.prompt.executor.clients.bedrock.modelfamilies.BedrockToolConverter
...
internal object BedrockAnthropicClaudeConverter {
    ...
    internal fun createAnthropicRequest(
                        ...
                        AnthropicMessage(
                            ...
                            content = listOf(
                                AnthropicContent.ToolUse(
                                    ...
                                    name = msg.tool,
                                    input = bedrockJson.parseToJsonElement(msg.content).jsonObject
                                )
                            )
                        )
                    )
                    ...
                    inputSchema = AnthropicToolSchema(
                        properties = buildJsonObject {
                            (tool.requiredParameters + tool.optionalParameters).forEach { param ->
                                put(param.name, BedrockToolConverter.buildToolParameterSchema(param))
                            }
                        },
        ...
        // Tool choice is not available in the Harmony format yet
    ...
    internal fun parseAnthropicResponse(responseBody: String, clock: Clock = Clock.System): List<Message.Response> {
        val response = bedrockJson.decodeFromString<AnthropicResponse>(responseBody)
    ...
    internal fun parseAnthropicStreamChunk(chunkJsonString: String): String {
        val streamResponse = bedrockJson.decodeFromString<AnthropicStreamResponse>(chunkJsonString)
